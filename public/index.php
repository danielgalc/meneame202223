...
<body>
    ...
    $pdo = conectar();
    $sent = $pdo->query("SELECT * FROM noticias ORDER BY id");
    ?>
            ...
            <main class="flex-1 grid grid-cols-3 gap-4 justify-center justify-items-center">
                <?php foreach ($sent as $fila) : ?>
                    <div class="p-6 max-w-xs min-w-full bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400"><?= hh($fila['noticia_usuario']) ?></p>
                        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400"><?= hh($fila['likes']) ?> ♡</p>
                        <a href="/insertar_en_carrito.php?id=<?= $fila['id'] ?>" class="w-32 inline-flex justify-center py-2 px-3.5 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            Leer más
