...
<head>
    ...
    <title>Gestión desde administrador</title>
...
<body>
    ...
    <div class="w-full flex justify-center items-center">
        <button type="button" class="my-10 bg-pink-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full" type="button" data-modal-toggle="insertar">Usuarios</button>
    </div>
    ...
    <div class="w-full flex justify-center items-center">
        <table>
            <thead class="border-b">
                ...
                <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-center">Likes</th>
                <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-center">ID Usuario</th>
                <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4 text-center">Acciones</th>
            </thead>
            <tbody>
                <?php foreach ($sent as $fila) : ?>
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-left"><?= $fila['titular'] ?></td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center"><?= $fila['likes'] ?> ♡</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center"><?= $fila['noticia_usuario'] ?></td>
    ...
    $pdo = conectar();
    $sent = $pdo->query("SELECT * FROM usuarios ORDER BY id ASC");
    ?>
    ...
    <div class="w-full flex justify-center items-center">
        <table>
            ...
            <tbody>
                <?php foreach ($sent as $fila) : ?>
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-left"><?= $fila['id'] ?></td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center"><?= $fila['usuario'] ?> ♡</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center"><?= $fila['password'] ?></td>
                        <td class="flex text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                            <form action="borrUsuario.php">
                                <?php var_dump($fila['id']) ?>
                                <a href="/admin/borrUsuario.php?id=<?= $fila['id'] ?>"><button type="button" class="bg-red-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">Borrar</button></a>
                            </form>
                            <form action="/admin/modUsuario.php" method="get" class="inline">
                                <input type="hidden" name="id" value="<?= $fila['id'] ?>">
                                <button type="button" class="bg-green-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full" data-modal-toggle="modUsuario">Modificar</button>
                                <a href="/admin/modUsuario.php?id=<?= $fila['id']?>&usuario=<?= obtener_post('usuario') ?>">Enviar Cambios</a>
                            </form>
                        </td>
                    </tr>
                <?php endforeach ?>
    ...
    <!-- Main modal -->
    <div id="modUsuario" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
        <div class="relative w-full h-full max-w-md md:h-auto">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-toggle="modUsuario">
                    <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                    </svg>
                    <span class="sr-only">Cerrar</span>
                ...
                <div class="px-6 py-6 lg:px-8">
                    ...
                    <form class="space-y-6" action="/admin" method="post">
                        <div class="mb-2">
                            <label for="titular" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Titular</label>
                            <input type="text" name="usuario" id="usuario" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Nuevo nombre de usuario" required>
                            <input type="hidden" id="hidden" name="id">
                        </div>
                        <button type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Modificar usuario</button>
                    </form>
                </div>
            </div>
        </div>
